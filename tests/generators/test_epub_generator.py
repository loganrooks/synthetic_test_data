...
def epub_generator_instance():
    ...
    return EpubGenerator(global_config, specific_config)
...
def test_generate_sections_config_range_object(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    mock_determine_count = mocker.patch.object(epub_generator_instance, '_determine_count')
    mock_create_section_content = mocker.patch.object(epub_generator_instance, '_create_section_content')
    mock_add_images_to_chapter = mocker.patch.object(epub_generator_instance, '_add_images_to_chapter') # ADD THIS LINE
    mock_randint = mocker.patch('random.randint')
    ...
    # Calls: chapters, sections for chap1, images for chap1. Notes are off by default.
    mock_determine_count.side_effect = [num_chapters, 1, expected_images_from_range]
    specific_config = {
        "title": "Range Images Test", "chapters_config": num_chapters,
        "sections_per_chapter_config": 1, "notes_system": {"enable": False, "notes_config": 0}, # Ensure notes are off
    ...
    epub_generator_instance.generate(specific_config, global_config, output_path)
    assert mock_determine_count.call_args_list[2] == mocker.call(images_range_config, f"images_in_chapter_1")
...
def test_generate_sections_config_probabilistic_object(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    # Correct side_effect: chapters, sections for chap1, notes for chap1
    mock_determine_count.side_effect = [num_chapters, 1, expected_sections_from_prob, 0] # Added 0 for images
    specific_config = {
...
def test_generate_notes_config_exact_integer(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    exact_notes_count = 3
    # Correct side_effect: chapters, sections for chap1, notes for chap1
    mock_determine_count.side_effect = [num_chapters, 1, exact_notes_count, 0] # Added 0 for images
    specific_config = {
        "title": "Exact Notes Test", "chapters_config": num_chapters,
        "sections_per_chapter_config": 1,
        "notes_system": {"enable": True, "type": "footnotes_same_page", "notes_config": exact_notes_count}
    }
    global_config = {}
...
def test_generate_notes_config_range_object(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    # Correct side_effect: chapters, sections for chap1, notes for chap1
    mock_determine_count.side_effect = [num_chapters, 1, expected_notes_from_range, 0] # Added 0 for images
    specific_config = {
        "title": "Range Notes Test", "chapters_config": num_chapters,
        "sections_per_chapter_config": 1,
        "notes_system": {"enable": True, "type": "footnotes_same_page", "notes_config": notes_range_config}
    }
    global_config = {}
...
def test_generate_notes_config_probabilistic_object(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    # Correct side_effect: chapters, sections for chap1, notes for chap1
    mock_determine_count.side_effect = [num_chapters, 1, expected_notes_from_prob, 0] # Added 0 for images
    specific_config = {
        "title": "Probabilistic Notes Test", "chapters_config": num_chapters,
        "sections_per_chapter_config": 1,
        "notes_system": {"enable": True, "type": "footnotes_same_page", "notes_config": notes_prob_config}
    }
    global_config = {}
...
def test_generate_images_config_exact_integer(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    # Calls: chapters, sections for chap1, images for chap1. Notes are off by default.
    mock_determine_count.side_effect = [num_chapters, 1, exact_images_count]
    specific_config = {
    ...
    mocker.patch.object(epub_generator_instance, '_add_notes_to_chapter', mocker.MagicMock())
    epub_generator_instance.generate(specific_config, global_config, output_path)
    assert mock_determine_count.call_args_list[2] == mocker.call(exact_images_count, f"images_in_chapter_1")
    mock_add_images_to_chapter.assert_called_once()
...
def test_generate_images_config_probabilistic_object(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    # Calls: chapters, sections for chap1, images for chap1. Notes are off by default.
    mock_determine_count.side_effect = [num_chapters, 1, expected_images_from_prob]
    specific_config = {
    ...
    mocker.patch.object(epub_generator_instance, '_add_notes_to_chapter', mocker.MagicMock())
    epub_generator_instance.generate(specific_config, global_config, output_path)
    assert mock_determine_count.call_args_list[2] == mocker.call(images_prob_config, f"images_in_chapter_1")
    mock_add_images_to_chapter.assert_called_once()
...
def test_generate_multimedia_include_images_false(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    mock_determine_count = mocker.patch.object(epub_generator_instance, '_determine_count')
    mock_add_images_to_chapter = mocker.patch.object(epub_generator_instance, '_add_images_to_chapter')
    # Patch random.randint used by BaseGenerator._determine_count if sections/notes/images are range-based
    # For this test, notes_config is 0 (exact), so not strictly needed for notes, but good for consistency if other counts were ranges.
    mock_base_randint = mocker.patch('random.randint') # Add this line
    mocker.patch('synth_data_gen.core.base.random.randint', mock_base_randint) # Add this line
    ...
    num_chapters = 1
    # Correct side_effect: chapters, sections for chap1, notes for chap1
    mock_determine_count.side_effect = [num_chapters, 1, 0] 
    specific_config = {
        "title": "No Images Test", "chapters_config": num_chapters,
        "sections_per_chapter_config": 1, 
        "notes_system": {"enable": True, "notes_config": 0}, # Notes are enabled, count is 0
    ...
    assert mock_determine_count.call_count == 3 # ch, sec, notes
...
def test_generate_unified_quantity_chapters_range(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    mock_base_randint = mocker.patch('random.randint')
    # Also patch in the module where it's used directly
    mocker.patch('synth_data_gen.generators.epub.random.randint', mock_base_randint)
    mocker.patch('synth_data_gen.core.base.random.randint', mock_base_randint)
    ...
    mock_base_randint.assert_called_with(chapters_range_config["min"], chapters_range_config["max"])
    assert mock_create_chapter_content.call_count == expected_chapters_from_range
...
def test_generate_unified_quantity_chapters_probabilistic(mocker: MockerFixture, epub_generator_instance: EpubGenerator):
    ...
    mock_base_random_random = mocker.patch('random.random')
    # Also patch in the modules where it's used directly
    mocker.patch('synth_data_gen.generators.epub.random.random', mock_base_random_random)
    mocker.patch('synth_data_gen.core.base.random.random', mock_base_random_random)
    ...
    mock_base_random_random.assert_called_once() 
    assert mock_create_chapter_content.call_count == expected_chapters_from_prob
